...
const Login = () => {
  ...
  // Real-time validation for QR email
  useEffect(() => {
    const errors = {};
    
    if (qrEmailInput) {
      const emailValidation = validateEmail(qrEmailInput);
      if (!emailValidation.isValid()) {
        errors.qrEmail = emailValidation.getFirstError();
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!username.trim()) {
      ...
      toast.showToast('error', 'Đăng nhập thất bại', 'Tên đăng nhập không được để trống');
      return;
    }
    
    if (!password) {
      setError('Mật khẩu không được để trống');
      toast.showToast('error', 'Đăng nhập thất bại', 'Mật khẩu không được để trống');
      return;
    }
  ...
  const handleFetchQr = async () => {
    ...
    const emailValidation = validateEmail(qrEmailInput);
    if (!emailValidation.isValid()) {
      setQrError(emailValidation.getFirstError());
      toast.showToast('error', 'Lỗi', emailValidation.getFirstError());
      return;
    }
    ...
    setQrError('');
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/users/qr-code-by-email?email=${encodeURIComponent(qrEmailInput.trim())}`);
      setQrToken(res.data);
      setQrEmail(qrEmailInput.trim());
      toast.showToast('success', 'Thành công', 'Mã QR đã sẵn sàng. Hãy quét bằng điện thoại đã đăng nhập.');
    } catch (err) {
      const msg = err.response?.data || 'Email không tồn tại trong hệ thống';
      setQrError(msg);
                    ...
                    <div className="col-md-6 d-none d-md-flex flex-column align-items-center justify-content-center bg-light bg-opacity-50 border-start">
                      <div className="text-center p-5">
                        ...
                        <div className="input-group mb-2 shadow-sm rounded-3 overflow-hidden">
                          <input
                            type="email"
                            className={`form-control border-0 ${validationErrors.qrEmail ? 'is-invalid' : ''}`}
                            placeholder="Email tài khoản"
                            value={qrEmailInput}
                            onChange={e => setQrEmailInput(e.target.value)}
                          />
                          <button 
                            className="btn btn-primary-blue px-3 border-0" 
                            onClick={handleFetchQr} 
                            disabled={qrLoading || validationErrors.qrEmail}
                          >
                            {qrLoading ? <span className="spinner-border spinner-border-sm"></span> : <Search size={18} />}
                          </button>
                        </div>
                        {validationErrors.qrEmail && (
                          <div className="text-danger small text-start mb-3">{validationErrors.qrEmail}</div>
                        )}
                        {qrError && (
                          <div className="text-danger small text-start mb-3">{qrError}</div>
                        )}
